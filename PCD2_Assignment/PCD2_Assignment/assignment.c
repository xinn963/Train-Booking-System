...
typedef struct {
	char receiptId[9]; // Receipt ID as a string
		...
		if (strcmp(temp[i].memberID, (*read).memberID) == 0) {
			do {
				...
				case 2:
					...
					printf("input name : %s", input.name);
					break;
...
void display() {
	...
	while (fscanf(display, "%[^|]|%[^|]|%[^|]|%[^|]|%d/%d/%d|%d|%d|%d|%lf\n",
		...
		printf("%-10s %-12s %-19s %-17s %02d/%02d/%d          %04d             %04d             %d         %.2lf\n",
			displayTrain.trainId, displayTrain.trainType, displayTrain.departureStation, displayTrain.arrivalStation, displayTrain.departureDate.day,
			displayTrain.departureDate.month, displayTrain.departureDate.year, displayTrain.departureTime, displayTrain.arrivalTime,
			...
			if (strcmp(modifyTrains[i].trainId, input) == 0) {
				...
				if (toupper(confirmModify) == 'Y') {
					...
					case 3:
						...
						break;
					case 4:
						printf("Enter new available seat (1 - 100) : ");
						rewind(stdin);
...
void reportTrain() {
	...
	TrainScheduling reportTrain[100];
	...
	while (fscanf(report, "%[^|]|%[^|]|%[^|]|%[^|]|%d/%d/%d|%d|%d|%d|%lf\n",
		&reportTrain[i].trainId, &reportTrain[i].trainType, &reportTrain[i].departureStation, &reportTrain[i].arrivalStation, &reportTrain[i].departureDate.day,
		&reportTrain[i].departureDate.month, &reportTrain[i].departureDate.year, &reportTrain[i].departureTime, &reportTrain[i].arrivalTime,
		&reportTrain[i].availableSeat, &reportTrain[i].price) != EOF) {

		if (morningPeakHour(reportTrain[i].departureTime) || eveningPeakHour(reportTrain[i].departureTime)) {
			if (reportTrain[i].availableSeat > 50) {
				printf("%-10s %-12s %-19s %-17s   %02d/%02d/%d         %04d             %04d             %d        %.2lf\n",
					reportTrain[i].trainId, reportTrain[i].trainType, reportTrain[i].departureStation, reportTrain[i].arrivalStation, reportTrain[i].departureDate.day,
					reportTrain[i].departureDate.month, reportTrain[i].departureDate.year, reportTrain[i].departureTime, reportTrain[i].arrivalTime,
					reportTrain[i].availableSeat, reportTrain[i].price);

				FILE* generate;

				generate = fopen("report.txt", "a");
				...
				fprintf(generate, "%s|%s|%s|%s|%d/%d/%d|%d|%d|%d|%.2lf\n", reportTrain[i].trainId, reportTrain[i].trainType, reportTrain[i].departureStation,
					reportTrain[i].arrivalStation, reportTrain[i].departureDate.day, reportTrain[i].departureDate.month, reportTrain[i].departureDate.year,
					reportTrain[i].departureTime, reportTrain[i].arrivalTime, reportTrain[i].availableSeat, reportTrain[i].price);
	...
	do {
		printf("\nReturn to main page (Y = yes / N = No) ? ");
		rewind(stdin);
		scanf("%c", &cont);
